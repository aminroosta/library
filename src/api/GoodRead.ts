...
const URLs = {
  getByTitle: title => `https://www.goodreads.com/book/title?title=${title}&format=xml&key=${GOODREAD.key}`,
  searchByQuery: query => `https://www.goodreads.com/search/index.xml?q=${query}&key=${GOODREAD.key}]`,
};
